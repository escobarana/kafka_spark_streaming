...
json_options = {"maxFilesPerTrigger": "1", "cleanSource": "delete", "header": "true", "inferschema": "true"}
catalog = {
    ...
    "invoices_parquet": DataLink("csv", RESOURCES_DIR / "invoices-parquet", csv_options),
}


def load_frame_from_catalog(
    spark: SparkSession, format: str, dataset_name: str, catalog: Mapping[str, DataLink] = catalog
) -> DataFrame:
...
if __name__ == "__main__":
    ...
    spark = SparkSession \
        ...
        .getOrCreate()
    frame = load_frame_from_catalog(spark=spark, format="json", dataset_name="invoices_json")
    frame.printSchema()
